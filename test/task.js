describe ('task', function() {
	it("should be able to create a new task", function()
	{
		...
		let task = ultralite.task.create();
	...
	describe('resolution', function() {
		...
		{
			let task = ultralite.task.create({'baseRoll': 10});
		...
		{
			let stub = sinon.stub(ultralite.task, "roll").returns(3);
			// the task would almost always fail, since it's at total -2
			let task = ultralite.task.create({"baseRoll": 8, "modifier": -10});
			let taskResolution = task.resolve();
		...
		{
			let stub = sinon.stub(ultralite.task, "roll").returns(18);
			// the task would almost always succeed, since a roll of 28 or less would succeed
			let task = ultralite.task.create({"baseRoll": 18, "modifier": 10});
			let taskResolution = task.resolve();
		...
		{
			let stub = sinon.stub(ultralite.task, "roll").returns(10);
			let task = ultralite.task.create();
			task.resolve();
		...
		{
			let stub = sinon.stub(ultralite.task, "roll").returns(11);
			let task = ultralite.task.create();
			task.resolve();
		...
		{
			let task = ultralite.task.create({"skill": "Knight"});
		...
		it("must correctly resolve a task with a positive modifier", function()
		{
			...
			let task = ultralite.task.create({"modifier": 2});
			task.resolve();
			task.roll.should.equal(11);
			task.status.should.equal(ultralite.task.status.success);
			stub.restore();
