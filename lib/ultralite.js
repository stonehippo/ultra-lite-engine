...
var ultralite = {
	dice: {
		rollWithRange: function(min, max) {
			if (min == null) throw Error("/min value not specified");
			if (max == null) throw Error("/max value not specified");
		...
		rollWithDiceCountOf: function(dice){
			if(!dice) throw RangeError("Number of dice must be at least 1");
