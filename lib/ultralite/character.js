...
const character = {
		...
		const that = {
			...
			setAttribute(attribute, value) {
				if (Object.getOwnPropertyNames(attributeLevels)
					...
					&& Object.prototype.hasOwnProperty.call(attributeLevels[attribute], value)
				) {
			...
			setAttributeLevel(attribute, level) {
				if (Object.getOwnPropertyNames(attributeLevels)
					...
					for (const attributeLevel in attributeLevels[attribute]) {
	...
	loadCharactersFromLocalFile: function(filePath) {
		const characterFile = readFileSync(filePath, "utf8");
		const characters = JSON.parse(characterFile);
